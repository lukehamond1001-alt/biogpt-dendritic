...
Standard Transformer:         BioGPT:
  ...
  FFN                                -> NMDA Trace
  Residual                           -> Causal Temporal Conv1d
                                     -> Per-Branch FFN (Grouped Conv1d)
...
### 2. NMDA Temporal Trace

**Biological analog**: NMDA receptors on dendrites have a characteristically slow decay time (~100ms), allowing them to integrate signals across time. This gives neurons a form of short-term memory that persists beyond the immediate input.

**Implementation**: A learnable exponential moving average that enriches the Value (V) tensor in attention:

```python
# tau is learned per-branch
V_enriched = V + nmda_trace * sigmoid(tau)
new_trace = tau * old_trace + (1 - tau) * current_output
```

**Current limitation**: During training with gradient checkpointing, temporal states cannot be propagated between chunks. The NMDA parameters are learned but the trace is always initialized to zero for each forward pass. This means the NMDA mechanism primarily contributes through its learned parameters rather than actual temporal integration during training. Fixing this requires custom checkpoint-compatible state management.

### 3. Causal Temporal Conv1d
...
### 4. Per-Branch FFN (Grouped Conv1d)
...
### 6. Soma Integration + Per-Neuron FFN
...
### 7. CorticalLayer (Multiple Neurons)
...
### 9. Gated Feedback Connections

**Biological analog**: Top-down feedback from deeper cortical layers (Layer 5/6) back to earlier processing stages (Layer 2/3). This creates expectation-based modulation of early processing.

**Implementation**: Late layers (8-11) send feedback to early layers (0-3) through a learned gated connection:

```python
gate = sigmoid(Linear(concat(target, feedback)))
output = target + gate * LayerNorm(Linear(feedback))
```

The sigmoid gate allows the model to learn how much top-down information to incorporate. This is currently used only in multi-pass mode (n_passes > 1).
...
| Feedback connections | ~4.5M | 4 gated connections |
| **Total** | **~294M** | |
